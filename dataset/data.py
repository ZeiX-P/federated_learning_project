...
class Dataset: # Keeping the class name as 'Dataset' as per your provided code
    ...
    def _validate_iid_split(self, dataset, indices_clients, num_clients):
        ...
        overall_dist_dict = {f"overall_class_{cls}_proportion": dist 
                            for cls, dist in zip(unique_classes, overall_dist)}
        wandb.log({"data_split/class_distribution": overall_dist_dict})
        ...
        for client_id in range(num_clients):
            ...
            for cls in unique_classes:
                ...
                client_class_dist[f"class_{cls}"] = client_prop
            
            deviations.append(deviation)
            client_distributions[f"client_{client_id}"] = client_class_dist
            ...
            if client_id < min(10, num_clients):  # Log first 10 clients
                wandb.log({
                    ...
                    f"data_split/client_{client_id}_deviation": deviation,
                    **{f"data_split/client_{client_id}_{k}": v for k, v in client_class_dist.items()}
                })
        ...
        deviation_data = [[i, dev] for i, dev in enumerate(deviations)]
        ...
        if num_clients <= 50:  # Only create heatmap for reasonable number of clients
            ...
            for client_id in range(num_clients):
                client_labels = labels[indices_clients[client_id]]
                for cls in unique_classes:
                    client_prop = np.sum(client_labels == cls) / len(client_labels)
                    heatmap_data.append([client_id, cls, client_prop])
        ...
        summary_data = [
            ...
            ["Max Deviation", f"{max_deviation:.4f}"],
            ["Mean Deviation", f"{mean_deviation:.4f}"],
            ["Quality", quality]
        ]
        summary_table = wandb.Table(data=summary_data, columns=["Metric", "Value"])
        wandb.log({"data_split/summary": summary_table})
